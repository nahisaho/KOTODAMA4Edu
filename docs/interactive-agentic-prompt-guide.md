...
flowchart TD
    BOOK["📖 本記事の構成"] --> C2
    C2["第2章: 設計思想と基本原則"] --> C2D["なぜこの設計が効果的なのか、理論的背景を理解"]
    C2 --> C3
    C3["第3章: 5フェーズ構造の詳細解説"] --> C3D["実装の核となるアーキテクチャを習得"]
    C3 --> C4
    C4["第4章: 理論・フレームワーク統合の設計"] --> C4D["専門知識をどう組み込むかを学ぶ"]
    C4 --> C5
    C5["第5章: 実践例（4つの教育系プロンプト）"] --> C5D["具体的な実装例で理解を深める"]
    C5 --> C6
    C6["第6章: Interactive Agentic Prompt を使う価値"] --> C6D["導入効果とROIを把握"]
    C6 --> C7
    C7["第7章: 自分だけのIAPを作る"] --> C7D["実践的な作成ガイド"]
    C7 --> C8
    C8["第8章: まとめと今後の展望"] --> C8D["次のステップへ"]
...
人間の認知負荷を考慮した設計です。複数の質問を同時に投げかけると：

```mermaid
flowchart LR
    subgraph BEFORE["❌ 従来のアプローチ（認知負荷が高い）"]
        direction TB
        Q["以下の質問にお答えください"]
        Q1["1. 対象学年は何年生ですか？"]
        Q2["2. 教科は何ですか？"]
    ...
    RESULT1["😵 ユーザーは圧倒され<br/>回答の質が低下する"]
    BEFORE --> RESULT1
    
    style BEFORE fill:#ffcccc,stroke:#cc0000
    style RESULT1 fill:#ff9999,stroke:#cc0000
```

```mermaid
flowchart TB
    subgraph AFTER["✅ Interactive Agentic Prompt のアプローチ"]
        direction TB
        Q1["まず、対象となる学年を<br/>教えていただけますか？"]
    ...
    RESULT2["😊 回答しやすく<br/>AIも次の質問を最適化できる"]
    AFTER --> RESULT2
    
    style AFTER fill:#ccffcc,stroke:#00cc00
    style RESULT2 fill:#99ff99,stroke:#00cc00
```
...
- **ミラーの法則**: 人間のワーキングメモリは7±2項目が限界
- **認知負荷理論**: 外在的認知負荷を減らすことで学習効率が向上
...
```mermaid
flowchart TB
    subgraph PHASE1_3["📋 Phase 1-3 で収集した情報"]
        INFO1["🎯 ユーザーの目標:<br/>小学4年生向け理科の授業設計"]
        INFO2["⚠️ 制約: 45分授業、<br/>実験器具なし、タブレット利用可"]
        INFO3["❗ 課題: 生徒の集中力が続かない"]
        INFO4["📚 選択理論: 構成主義学習理論、<br/>ガニェの9教授事象"]
    end
    
    subgraph PHASE4["⚙️ Phase 4: メタプロンプト生成"]
        direction TB
        ROLE["あなたは構成主義学習理論に基づく<br/>授業設計の専門家です。"]
        ...
        ROLE --> CONDITIONS --> OUTPUT
    end
    
    subgraph PHASE5["✨ Phase 5: 最適化された応答"]
        RESULT["文脈に完全に適合した<br/>理論に基づいた授業案"]
    end
    
    PHASE1_3 --> PHASE4 --> PHASE5
    
    style PHASE1_3 fill:#e6f3ff,stroke:#0066cc
    style PHASE4 fill:#fff2e6,stroke:#cc6600
    style PHASE5 fill:#e6ffe6,stroke:#00cc00
```
...
理論・フレームワークを明示的に組み込むことで：

```mermaid
flowchart TB
    subgraph EFFECT["🔄 理論・フレームワーク統合の効果"]
        direction TB
        
        subgraph BEFORE["❌ Before: 理論なしの応答"]
            B1["「効果的な授業のために、<br/>以下の点に注意してください...」"]
            B2["→ 一般論、根拠不明、再現性低い"]
        end
        
        subgraph AFTER["✅ After: 理論に基づく応答"]
            A1["「構成主義学習理論に基づき、<br/>生徒が自ら知識を構築する<br/>プロセスを設計します。<br/>ガニェの9教授事象を適用し...」"]
            A2["→ 根拠明確、実践的、再現性高い"]
        end
    end
...
```mermaid
flowchart TB
    subgraph Structure["理論統合の3層構造"]
        L1["Layer 1: 理論・フレームワーク・レポジトリ"]
        L1D["ドメイン固有の理論を体系的に整理<br/>例: 学習理論、動機づけ理論、発達理論、etc."]
        L1 --> L1D
        L1D --> L2
        
        L2["Layer 2: 選択ガイドライン"]
        L2D["どのような状況でどの理論を選択すべきかの指針<br/>例: フローチャート、意思決定マトリクス"]
        L2 --> L2D
        L2D --> L3
        
        L3["Layer 3: 適用テンプレート"]
        L3D["選択した理論をどう具体的に適用するかのパターン<br/>例: 理論Xを使った授業設計テンプレート"]
        L3 --> L3D
    end
    
    style L1 fill:#1976d2,color:#fff,stroke-width:2px
    style L2 fill:#388e3c,color:#fff,stroke-width:2px
    style L3 fill:#f57c00,color:#fff,stroke-width:2px
    style L1D fill:#e3f2fd,stroke:#1976d2
    style L2D fill:#e8f5e9,stroke:#388e3c
    style L3D fill:#fff3e0,stroke:#f57c00
```

この構造により、AIは「どの理論を」「なぜ」「どう適用するか」を明確に判断できるようになります。

---

> **💡 ポイント**
> 
> Interactive Agentic Prompt の設計思想は、以下の4つの柱で成り立っています。
...
ここでは、実際に設計・実装した4つの Interactive Agentic Prompt を紹介します。それぞれが異なる専門領域をカバーし、豊富な理論・フレームワークを統合しています。
...
```mermaid
flowchart TB
    S1["Step 1: ドメインと対象ユーザーの定義"] --> S1D["誰のための、何についてのプロンプトか"]
    S1D --> S2
...
| リソース | URL | 説明 |
|---------|-----|------|
| **本プロジェクト** | GitHub | 4つの教育系プロンプトのソースコード |
